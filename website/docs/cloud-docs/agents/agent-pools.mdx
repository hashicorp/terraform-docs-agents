...
An agent pool in HCP Terraform is a collection of agents that can be assigned to workspaces to handle Terraform operations for workspaces. 
These agents are responsible for executing Terraform runs within specific, scoped workspaces. Agent pools help distribute the execution of Terraform 
workloads across multiple agents, ensuring scalability and flexibility.

## Permissions

To manage agent pools, you must have the following permissions:

- **Owners** team membership to manage agents at the organization level.
- **Admin** access to a workspace to change its [execution mode](/terraform/cloud-docs/workspaces/settings#execution-mode) to use an agent pool.
...
To connect an agent to this pool, configure and [start the official agent Docker container](/terraform/cloud-docs/agents/agents#quick-start).
Alternatively, start an agent [from the downloaded agent file](/terraform/cloud-docs/agents/agents#start-an-agent) and provide it with the token for this agent pool.

## Scope an Agent Pool to Specific Workspaces

Scoping an agent pool ensures that only selected workspaces can use it. The pool will only appear in the list of available agent pools within the chosen workspaces.
...
The pool will only be accessible by the selected workspaces.

## Configure Workspaces to use the Agent

To configure a workspace to use an agent pool, follow these steps:
