...
These agents are responsible for executing Terraform runs within specific, scoped workspaces. 
Agent pools help distribute the execution of Terraform workloads across multiple agents, ensuring scalability and flexibility.
...
Alternatively, [start an agent from an agent file](/terraform/cloud-docs/agents/agents#start-an-agent) and provide it with the token for this agent pool.
