...
* Resolved a bug where the use of a `terraform-bundle` in combination with a defined working directory resulted in the following error: `Operation failed: failed packing filesystem: illegal slug error: invalid symlink`.
