...
>   Install and run HCP Terraform agents with the official agent Docker container.
    The `TFC_AGENT_TOKEN` field is generated when you create an agent pool.
    ```
    docker pull hashicorp/tfc-agent:latest
    docker run -e TFC_AGENT_TOKEN=your-token -e TFC_AGENT_NAME=your-agent-name TFC_ADDRESS=your-address hashicorp/tfc-agent
    ```
